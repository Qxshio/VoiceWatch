...
- Desktop app sends a rejoin command only after the user clicks a rejoin button.
- Extension uses Roblox's page launcher only on the resulting marked rejoin page
  and only with the explicit sanitized metadata in that URL.
- Desktop app may use a Roblox app deep link only as a local fallback when the
  browser command cannot be queued.
